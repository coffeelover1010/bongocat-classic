...
- `/bc` — show help.
- `/bc config` — toggle either cat, optionally limit the chat cat to when its input is open (without changing which cat is active), cycle compact T-shirt sizes (XS–XL), open native colour pickers for fill and outline, choose from Background through On top UI layers, lock its drag position, open the action-trigger list to select triggers individually (which collapses other settings), adjust inactivity fade settings, and set the global action sequence length and tap interval.
