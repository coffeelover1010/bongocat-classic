...
- `/bc` — show help.
- `/bc config` — use the short explanations to set up chat, action, and spell cats. Trigger lists are separate compact views; spell triggers include cast bars, channels, instant spells, buffs, and debuffs. Spell settings also offer opacity, XXL/XXXL sizes, post-sequence visibility time, icon zoom, a border toggle, and cat-to-icon offsets.
